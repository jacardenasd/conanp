...
$id = $_POST['id'];
$stmt = $pdo->prepare("UPDATE aportaciones_destacadas SET validado_rh = 1, rechazado_por_rh = 0 WHERE id = ?");
$stmt->execute([$id]);
header("Location: admin_aportaciones_destacadas.php?info=1&colaborador_id=$colaborador_id&periodo=$periodo_actual");
exit();
}
if (isset($_POST['rechazar'])) {
$id = $_POST['id'];
$stmt = $pdo->prepare("UPDATE aportaciones_destacadas SET validado_rh = 0, rechazado_por_rh = 1 WHERE id = ?");
$stmt->execute([$id]);
header("Location: admin_aportaciones_destacadas.php?info=2&colaborador_id=$colaborador_id&periodo=$periodo_actual");
exit();
}
}

$sql = "SELECT a.*, CONCAT(TRIM(us.nombre), ' ', TRIM(us.apellido_paterno), ' ', TRIM(us.apellido_materno)) AS nombre_completo
		FROM aportaciones_destacadas a
		INNER JOIN usuarios us ON us.user_id = a.user_id
		WHERE a.periodo = ?";
$paramsAportaciones = [$periodo_actual];

if ($colaborador_id != '') {
	$sql .= " AND a.user_id = ?";
	$paramsAportaciones[] = $colaborador_id;
}
if ($unidad_id !== '') {
	$sql .= " AND us.unidad_id = ?";
	$paramsAportaciones[] = (int)$unidad_id;
}

$stmt = $pdo->prepare($sql);
$stmt->execute($paramsAportaciones);
...
$sqlUsuarios = "SELECT user_id, CONCAT(nombre, ' (', RFC, ')') AS nombre
				FROM usuarios
				WHERE 1";
$paramsUsuarios = [];
if ($unidad_id !== '') {
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM calificaciones WHERE user_id = ? AND periodo = ? AND estatus_aportaciones_destacadas = ?");
$stmt->execute([$colaborador_id, $periodo_actual, 0]);
$tiene_aportaciones = $stmt->fetchColumn();
...
if (isset($_GET['borrar']) AND $_GET['borrar'] == 1) { 
    ...
    $stmt = $pdo->prepare("SELECT user_id, periodo FROM aportaciones_destacadas WHERE id = ?");
    $stmt->execute([$id]);
    ...
    if ($registro) {
        ...
        // Recalcular: contar cuántas aportaciones quedan
        ...
        if ($count == 0) {
            ...
            $stmt = $pdo->prepare("UPDATE calificaciones SET estatus_aportaciones_destacadas = 0, aportaciones_destacadas = 0 WHERE user_id = ? AND periodo = ?");
            $stmt->execute([$affected_user_id, $affected_periodo]);
        } else {
            ...
            if ($validadas == 0) {
                $stmt = $pdo->prepare("UPDATE calificaciones SET estatus_aportaciones_destacadas = 1, aportaciones_destacadas = 0 WHERE user_id = ? AND periodo = ?");
                $stmt->execute([$affected_user_id, $affected_periodo]);
            } else {
                // Actualizar sumatoria
                $stmt = $pdo->prepare("UPDATE calificaciones SET aportaciones_destacadas = ? WHERE user_id = ? AND periodo = ?");
                $stmt->execute([$validadas, $affected_user_id, $affected_periodo]);
            }
        }
    }

    header("Location: admin_aportaciones_destacadas.php?info=3");
    exit;
}
?>
...
<?php require_once('assets/main_navbar.php'); ?>
	...
	<form class="row g-3 mb-3" method="get" action="admin_aportaciones_destacadas.php">
		<div class="col-md-4">
		...
		<div class="col-md-2 d-flex align-items-end">
			...
			<a href="admin_aportaciones_destacadas.php" class="btn btn-sm btn-secondary">Limpiar</a>
        </div>
    </form>
				...
				<div class="modal-dialog modal-lg">
					...
					<div class="modal-header bg-warning text-dark">
						<h5 class="modal-title">Rechazar Aportación Destacada</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
					</div>
					<div class="modal-body">
						¿Estás seguro de que deseas rechazar la Aportación: <br/>
						<strong><?= $u['descripcion'] ?></strong>?
					</div>
					<div class="modal-footer">
				...
				<div class="modal-dialog modal-lg">
					...
					<div class="modal-body">
						¿Estás seguro de que deseas eliminar la Aportación: <br/>
						<strong><?= $u['descripcion'] ?></strong>?
					</div>
					<div class="modal-footer">
              			<a href="admin_aportaciones_destacadas.php?id=<?= $u['id'] ?>&borrar=1" class="btn btn-sm btn-danger">Eliminar</a>
						<button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancelar</button>
					</div>
					</div>
				</div>
