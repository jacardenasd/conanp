...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['guardar'])) {
        ...
        if ($nombre !== '') {
            $stmt = $pdo->prepare("INSERT INTO ejes_pnd (nombre, orden, estatus) VALUES (?, ?, ?)");
            $stmt->execute([$nombre, $orden, $estatus]);
            header('Location: admin_ejes_pnd.php?mensaje=guardado');
            exit;
        }
    }
    ...
    if (isset($_POST['actualizar'])) {
        ...
        if ($id > 0 && $nombre !== '') {
            $stmt = $pdo->prepare("UPDATE ejes_pnd SET nombre = ?, orden = ?, estatus = ? WHERE id = ?");
            $stmt->execute([$nombre, $orden, $estatus, $id]);
            header('Location: admin_ejes_pnd.php?mensaje=actualizado');
            exit;
        }
    }
    ...
    if (isset($_POST['eliminar'])) {
        ...
        if ($id > 0) {
            $stmtUso = $pdo->prepare("SELECT COUNT(*) FROM metas_colectivas WHERE eje_pnd_id = ?");
            $stmtUso->execute([$id]);
            $enUso = (int)$stmtUso->fetchColumn();

            if ($enUso > 0) {
                header('Location: admin_ejes_pnd.php?mensaje=en_uso');
                exit;
            }

            $stmt = $pdo->prepare("DELETE FROM ejes_pnd WHERE id = ?");
            $stmt->execute([$id]);
            header('Location: admin_ejes_pnd.php?mensaje=borrado');
            exit;
        }
    }
}
...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM ejes_pnd e{$whereSql}");
...
$listSql = "SELECT e.*,
                   (SELECT COUNT(*) FROM metas_colectivas mc WHERE mc.eje_pnd_id = e.id) AS usos
            FROM ejes_pnd e{$whereSql}
        ...
        <div class="content-inner">
                ...
                <div class="page-header-content d-lg-flex border-top">
                    <div class="d-flex">
                        <div class="breadcrumb py-2">
                            ...
                            <span class="breadcrumb-item active">Ejes P.N.D.</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="content">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
                    <h2 class="mb-0">Administrar Ejes del P.N.D.</h2>
                </div>

                <?php if (isset($_GET['mensaje'])): ?>
                    <div class="alert <?= $_GET['mensaje'] === 'en_uso' ? 'alert-warning' : 'alert-success' ?>">
                        <?php
                        if ($_GET['mensaje'] === 'guardado') echo 'Eje guardado.';
                        if ($_GET['mensaje'] === 'actualizado') echo 'Eje actualizado.';
                        if ($_GET['mensaje'] === 'borrado') echo 'Eje eliminado.';
                        if ($_GET['mensaje'] === 'en_uso') echo 'No se puede eliminar: el eje está en uso en metas colectivas.';
                        ?>
                    </div>
                <?php endif; ?>
                ...
                <div class="card mb-3">
                    <div class="card-header"><h5 class="mb-0">Nuevo eje</h5></div>
                    <div class="card-body">
                        <form method="post" class="row g-3 align-items-end">
                            <div class="col-md-7">
                                <label for="nombre_nuevo" class="form-label">Nombre</label>
                                <input type="text" id="nombre_nuevo" name="nombre" class="form-control" placeholder="Ejemplo: Gobernanza con justicia y participación ciudadana." required>
                            </div>
                            <div class="col-md-2">
                    ...
                    <div class="card-body">
                        <form method="get" class="row g-3 align-items-end">
                            <div class="col-md-10">
                                ...
                                <input type="text" id="q" name="q" class="form-control" value="<?= htmlspecialchars($busqueda) ?>" placeholder="Texto del eje">
                            </div>
                            <div class="col-md-2 d-flex gap-2">
                                <button type="submit" class="btn btn-primary w-100">Filtrar</button>
                                <a href="admin_ejes_pnd.php" class="btn btn-light w-100">Limpiar</a>
                            </div>
                        </form>
                    </div>
                </div>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                                ...
                                                <div class="modal-content">
                                                    <form method="post">
                                                        <div class="modal-header bg-primary text-white">
                                                            <h6 class="modal-title">Editar eje</h6>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <form method="post">
                                                        ...
                                                        <div class="modal-body">
                                                            ¿Eliminar el eje <strong><?= htmlspecialchars($item['nombre']) ?></strong>?
                                                            <input type="hidden" name="id" value="<?= (int)$item['id'] ?>">
                                                        </div>
                                                        <div class="modal-footer">
                                    ...
                                    <?php if (count($items) === 0): ?>
                                        <tr><td colspan="6" class="text-center text-muted">No se encontraron ejes.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
