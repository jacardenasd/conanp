...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['agregar'])) {
        $stmt = $pdo->prepare("INSERT INTO metas (nombre, ponderacion, no_satisfactorio, unidad) VALUES (?, ?, ?, ?)");
        $stmt->execute([$_POST['nombre'], $_POST['ponderacion'], $_POST['no_satisfactorio'], $_POST['unidad']]);
    } elseif (isset($_POST['editar'])) {
        $stmt = $pdo->prepare("UPDATE metas SET nombre = ?, ponderacion = ?, no_satisfactorio = ?, unidad = ?  WHERE id = ?");
        $stmt->execute([$_POST['nombre'], $_POST['ponderacion'], $_POST['no_satisfactorio'], $_POST['unidad'], $_POST['id']]);
    }
...
// Enviar mensaje automático al jefe inmediato
$stmt = $pdo->prepare("SELECT jefe_id, nombre FROM usuarios WHERE user_id = ?");
$stmt->execute([$_SESSION['user_id']]);
$yo = $stmt->fetch();

if ($yo && $yo['jefe_id']) {
  $mensaje = "El usuario {$yo['nombre']} ha completado la captura de metas para el periodo {$periodo}.";

  $stmtMsg = $pdo->prepare("INSERT INTO mensajes (de_user_id, para_user_id, asunto, mensaje)
                            VALUES (?, ?, ?, ?)");
  $stmtMsg->execute([
    $_SESSION['user_id'],     // Remitente
    $yo['jefe_id'],         // Jefe inmediato
    "Captura de metas completada",
    $mensaje
  ]);
}


    header("Location: captura.php?info=1");
    exit();
...
$stmt = $pdo->query("SELECT * FROM metas ORDER BY id");
				...
				<div class="content">
					...
					<div class="card">
						<div class="card-header">
							<h5 class="mb-0">Metas Individuales</h5>
						</div>
						...
						<table class="table datatable-pagination">
							...
							<tbody>
								<?php foreach ($metas as $u): ?>
								<tr>
									<td><?= $u['id'] ?></td>
									<td><?= htmlspecialchars($u['nombre_meta']) ?></td>
									<td><?= htmlspecialchars($u['unidad']) ?></td>
									<td><?= htmlspecialchars($u['ponderacion']) ?>%</td>
									<td>
										<button class="btn btn-info btn-sm btn-editar" 
										data-id='<?= $u["id"] ?>' 
										data-nombre='<?= $u["nombre_meta"] ?>' 
										data-indicador='<?= $u["indicador"] ?>' 
										data-unidad='<?= $u["unidad"] ?>' 
										data-ponderacion='<?= $u["ponderacion"] ?>' 
										data-sobresaliente='<?= $u["sobresaliente"] ?>' 
										data-satisfactorio='<?= $u["satisfactorio"] ?>' 
										data-nosatisfactorio='<?= $u["no_satisfactorio"] ?>' 
										data-noaprobatorio='<?= $u["no_aprobatorio"] ?>' 
										data-deficiente='<?= $u["deficiente"] ?>' 
    ...
    <form method="post" class="modal-content">
      ...
      <div class="modal-body">
	  	...
	  	<div class="form-group">
            <label>Nombre</label>
            <input type="text" name="nombre" class="form-control" required placeholder="Nombre de unidad"></input>
        </div>
...
<div class="modal fade" id="modalEditar" tabindex="-1">
    ...
    <form method="post" class="modal-content">
      ...
      <div class="modal-header"><h5 class="modal-title">Editar Unidad</h5>
	  <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
	  </div>
      <div class="modal-body">
        ...
        <div class="form-group">
            <label>Unidad de Medida</label>
            <select  name="unidad" id="unidadEditar" class="form-control" required>
				<option value="">Seleccione...</option>
				<option value="cantidad">Cantidad</option>
                <option value="calidad">Calidad</option>
                <option value="tiempo">Tiempo</option>
...
$('.btn-editar').click(function() {
    ...
    $('#unidadEditar').val($(this).data('unidad'));
});
</script>
<!-- Modal Editar -->
...
<div class="modal fade" id="modalEliminar" tabindex="-1" aria-labelledby="modalEliminarLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
    <form method="get" action="captura.php" class="modal-content">
    <form method="post" class="modal-content">
      <input type="hidden" name="id" id="idEditar">
      <div class="modal-header"><h5 class="modal-title">Editar Unidad</h5>
	  <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
...
document.addEventListener("DOMContentLoaded", function() {
    document.querySelectorAll('.btn-eliminar').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('unidadEliminar').innerText = this.dataset.unidad;
