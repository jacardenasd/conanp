...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $totalmetas_colectivas = $stmt->fetchColumn();

  if ($totalmetas_colectivas >= 7) {
    $errores[] = 'Ya has capturado el máximo de 7 metas_colectivas.';
  }
  ...
  if (empty($errores)) {
    $stmt = $pdo->prepare("INSERT INTO metas_colectivas (unidad_id, periodo, indicador, unidad, ponderacion, sobresaliente, satisfactorio, no_satisfactorio, no_aprobatorio, deficiente, estatus)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
