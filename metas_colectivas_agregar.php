...
$unidades_medida = $pdo->query("SELECT nombre FROM unidades_medida ORDER BY nombre")->fetchAll(PDO::FETCH_COLUMN);
...
$periodo = $_POST['periodo'];
  ...
  if (empty($errores)) {
		$stmt = $pdo->prepare("INSERT INTO metas_colectivas (unidad_id, periodo, indicador, instrumento, eje_pnd_id, unidad, ponderacion, satisfactorio, estatus)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    header("Location: admin_metas_colectivas_detalle.php?info=1&unidad_id=$unidad_id&periodo=$periodo");
    exit;
  }
}
?>
            ...
            <?php endif; ?>
              ...
              <div class="row">
                  ...
                  <?php foreach ($unidades as $un): ?>
                    <option value="<?= $un['id'] ?>" <?php if (isset($_POST['id']) AND $_POST['id'] === $un['id']) {echo 'selected';} ?>><?= $un['nombre'] ?></option>
                  <?php endforeach; ?>
                </select>
              		</div>
               		</div>
                  ...
                  <?php foreach ($periodos as $pe): ?>
                    <option value="<?= $pe['anio'] ?>" <?php if (isset($_POST['anio']) AND $_POST['anio'] === $pe['anio']) {echo 'selected';} ?>><?= $pe['anio'] ?></option>
                  <?php endforeach; ?>
                </select>
                  </div>
               	</div>
