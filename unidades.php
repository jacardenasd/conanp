...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['agregar'])) {
        $stmt = $pdo->prepare("INSERT INTO unidades (nombre, ponderacion, no_satisfactorio, unidad) VALUES (?, ?, ?, ?)");
        $stmt->execute([$_POST['nombre'], $_POST['ponderacion'], $_POST['no_satisfactorio'], $_POST['unidad']]);
    } elseif (isset($_POST['editar'])) {
        $stmt = $pdo->prepare("UPDATE unidades SET nombre = ?, ponderacion = ?, no_satisfactorio = ?, unidad = ?  WHERE id = ?");
...
$stmt = $pdo->query("SELECT * FROM unidades ORDER BY id");
						...
						<table class="table datatable-pagination">
							...
							<tbody>
								...
								<tr>
									...
									<td><?= htmlspecialchars($u['no_satisfactorio']) ?></td>
									<td><?= htmlspecialchars($u['unidad']) ?></td>
									<td>
										<button class="btn btn-info btn-sm btn-editar" 
										data-id="<?= $u['id'] ?>" 
										data-nombre="<?= htmlspecialchars($u['nombre']) ?>" 
										data-ponderacion="<?= htmlspecialchars($u['ponderacion']) ?>" 
										data-no_satisfactorio="<?= htmlspecialchars($u['no_satisfactorio']) ?>" 
...
<div class="modal fade" id="modalEliminar" tabindex="-1" aria-labelledby="modalEliminarLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
    <form method="get" action="unidades.php" class="modal-content">
